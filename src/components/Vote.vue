<template>
  <div id="vote">
    ...
    <div class="overlay">
      ...
      <em class="blue">{{ nominator.name }}</em> nominerade <em>{{ nominee.name }}</em
      >!
      <br />
      <em class="blue"> {{ voters.length }} röst{{ voters.length !== 1 ? 'er' : '' }} </em>
      för
      <em v-if="nominee.role.team !== 'traveler'"> (majoritet är {{ Math.ceil(alive / 2) }}) </em>
      <em v-else>(majoritet är {{ Math.ceil(players.length / 2) }})</em>

      <template v-if="!session.isSpectator">
        <div v-if="!session.isVoteInProgress && session.lockedVote < 1">
          Tid per spelare:
          <font-awesome-icon @mousedown.prevent="setVotingSpeed(-500)" icon="minus-circle" />
        ...
        <div class="button-group">
          ...
          <template v-else>
            <div class="button townsfolk" :class="{ disabled: !session.lockedVote }" @click="pause">
              {{ voteTimer ? 'Paus' : 'Återstart' }}
            </div>
            <div class="button" @click="stop">Återställ</div>
          </template>
          <div class="button demon" @click="finish">Stäng</div>
        </div>
      ...
      <template v-else-if="canVote">
        <div v-if="!session.isVoteInProgress">{{ session.votingSpeed / 1000 }} sekunder mellan röster</div>
        <div class="button-group">
      ...
      <div v-else-if="!player">
        Vänligen ta en plats för att rösta.
      </div>
    </div>
...
export default {
  computed: {
    ...
    canVote: function() {
      ...
      if (this.player.isVoteless && this.nominee.role.team !== 'traveler') return false
    ...
    voters: function() {
      ...
      const voters = Array(this.players.length)
        .fill('')
        .map((x, index) => (this.session.votes[index] ? this.players[index].name : ''))
      const reorder = [...voters.slice(nomination + 1), ...voters.slice(0, nomination + 1)]
      return (this.session.lockedVote ? reorder.slice(0, this.session.lockedVote - 1) : reorder).filter((n) => !!n)
    },
  },
  data() {
    return {
