...
const getTravelersNotInEdition = (edition = editionJSON[0]) => {
  return new Map(
    rolesJSON
      .filter((r) => r.team === 'traveler' && r.edition !== edition.id && !edition.roles.includes(r.id))
      .map((role) => [role.id, role])
  )
}
...
// global data maps
const editionJSONbyId = new Map(editionJSON.map((edition) => [edition.id, edition]))
...
try {
  ...
  //   .then(jinxesJSON => {
  jinxes = new Map(
    jinxesJSON.map(({ id, hatred }) => [clean(id), new Map(hatred.map(({ id, reason }) => [clean(id), reason]))])
  )
  // });
} catch (e) {
...
export default new Vuex.Store({
  ...
  getters: {
    ...
    customRolesStripped: ({ roles }) => {
      ...
      const strippedProps = ['firstNightReminder', 'otherNightReminder', 'isCustom']
      roles.forEach((role) => {
        if (!role.isCustom) {
  ...
  mutations: {
    ...
    setCustomRoles(state, roles) {
      const processedRoles = roles
        ...
        .map((role) => rolesJSONbyId.get(role.id) || state.roles.get(role.id) || Object.assign({}, customRole, role))
      ...
      state.roles = new Map(processedRoles.filter((role) => role.team !== 'fabled').map((role) => [role.id, role]))
      ...
      state.otherTravelers = new Map(
        rolesJSON.filter((r) => r.team === 'traveler' && !roles.some((i) => i.id === r.id)).map((role) => [role.id, role])
      )
    },
