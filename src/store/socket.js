class LiveSession {
  ...
  _handleMessage({ data }) {
    ...
    switch (command) {
      ...
      case "clearVoteHistory":
        ...
        break;
      case "isPlayFanfare":
        this._store.commit("session/setPlayFanfare", params);
        break;
  ...
  sendGamestate(playerId = "", isLightweight = false) {
    ...
    if (isLightweight) {
      ...
      this._sendDirect(playerId, "gs", {
        ...
        isPlayFanfare: session.isPlayFanfare,
  ...
  _updateGamestate(data) {
    ...
    if (!isLightweight) {
      ...
      this._store.commit("session/setPlayFanfare", isPlayFanfare);
  ...
  /**
   * Set the isPlayFanfare status. ST only
   */
  setPlayFanfare() {
    if (this._isSpectator) return;
    this._send("isPlayFanfare", this._store.state.session.isPlayFanfare);
  }
  /**
